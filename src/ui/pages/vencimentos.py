...
class VencimentosPage(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # Filtro por NR
        self.nr_filter_var = ctk.StringVar(value="Todos")
        self.nr_filter = ctk.CTkOptionMenu(
            search_frame, variable=self.nr_filter_var,
            values=["Todos", "NR-01", "NR-06", "NR-12", "NR-18", "NR-26", "NR-35"],
            command=self._on_filter_change,
            font=fonts["body"], width=120, height=36, corner_radius=8,
            fg_color=COLORS["primary"], button_color=COLORS["secondary"]
        )
        self.nr_filter.grid(row=0, column=1)

        # Dashboard cards
        self.cards_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.cards_frame.grid(row=2, column=0, sticky="ew", padx=20, pady=(0, 5))
        for i in range(5):
        ...
        # Lista de certificados (scrollavel)
        self.list_frame = ctk.CTkScrollableFrame(
            self, fg_color=COLORS["surface"], corner_radius=10,
            border_width=1, border_color=COLORS["border"]
        )
        self.list_frame.grid(row=3, column=0, sticky="nsew", padx=20, pady=(5, 15))
    ...
    def _apply_filters(self):
        ...
        for cert in self.all_certs:
            if nr_filter != "Todos" and cert["nr_code"] != nr_filter:
                continue
            if search:
    ...
    def _render_list(self):
        ...
        row_idx = 0
        for (emp_id, emp_name, emp_cpf), certs in employees.items():
            self._create_employee_section(row_idx, emp_name, emp_cpf, certs)
            row_idx += 1

    def _create_employee_section(self, row_idx, name, cpf, certs):
        fonts = get_fonts()

        # Header do funcionario (clicavel para expandir/colapsar)
        ...
        for i, h in enumerate(headers):
            ctk.CTkLabel(
                table_frame, text=h,
                font=fonts["small_bold"], text_color=COLORS["text"], anchor="w"
            ).grid(row=0, column=i, padx=8, pady=(5, 2), sticky="w")
        ...
        for idx, cert in enumerate(sorted(certs, key=lambda c: c["dias_para_vencer"])):
            r = idx + 2
            bg = "#FFF3E0" if cert["status"] in ("vencido", "urgente") else "#F8FAFC"

            ctk.CTkLabel(
                table_frame, text=cert["nr_code"],
                font=fonts["body"], text_color=COLORS["text"], anchor="w"
            ).grid(row=r, column=0, padx=8, pady=3, sticky="w")

            ctk.CTkLabel(
                table_frame, text=cert["descricao_treinamento"][:35],
                font=fonts["body"], text_color=COLORS["text"], anchor="w"
            ).grid(row=r, column=1, padx=8, pady=3, sticky="w")
            ...
            if validade:
                ...
                validade_str = "-"
            ctk.CTkLabel(
                table_frame, text=validade_str,
                font=fonts["body"], text_color=COLORS["text"], anchor="w"
            ).grid(row=r, column=2, padx=8, pady=3, sticky="w")
            ...
            ctk.CTkLabel(
                table_frame, text=dias_text,
                font=fonts["body_bold"], text_color=dias_color, anchor="w"
            ).grid(row=r, column=3, padx=8, pady=3, sticky="w")

            # Status badge
            status_text = cert["status"].upper().replace("_", " ")
            badge = ctk.CTkLabel(
                table_frame, text=status_text,
                font=fonts["small_bold"],
                text_color=COLORS["surface"],
                fg_color=status_colors.get(cert["status"], COLORS["muted"]),
                corner_radius=4, padx=6, pady=2
            )
            badge.grid(row=r, column=4, padx=8, pady=3, sticky="w")

        # Ajustar row_idx para proximo funcionario
